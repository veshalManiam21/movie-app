...
export const useAnimationStyles = combinedStyles => {
  const animatedValuesRef = useRef({});
  const createAnimatedValueIfDoesntExist = useCallback((mapIndex, styleName) => {
  ...
  const animateStyle = useCallback((mapIndex, styleName, config) => {
    ...
    const easing = type === "bezier" ? Easing.bezier(args[0], args[1], args[2], args[3]) : Easing.linear; // Check if this value has any interpolation
    ...
    const defaultValue = getDefaultStyleValueFunction(styleName)(toValue); // Check if is object
  ...
  const transformStyles = useCallback((mapIndex, style) => {
    const {
      transitionProperty,
      transitionDuration,
  ...
  const requiresAnimatedComponent = useMemo(() => combinedStyles.some(combinedStyle => {
    if (combinedStyle) {
      return Object.keys(combinedStyle).some(key => ANIMATION_CONFIG_STYLE_PROPS.includes(key));
    }
