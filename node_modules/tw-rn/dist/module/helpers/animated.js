...
const defaultNumber = defaultValue => () => defaultValue;

const same = nextValue => nextValue;

const color = nextValue => {
  const color = tinycolor(nextValue);

  if (!color.isValid()) {
    throw new Error("Invalid color ".concat(nextValue));
  }

  return color.setAlpha(0).toRgbString();
};

const shadow = nextValue => ({
  width: 0,
  height: 0
});

export const getDefaultStyleValueFunction = styleName => {
  switch (styleName) {
